...
class AutoPurchaser:
    ...
    async def purchase(self, product_id, quantity=1, user_id=None, order_id=None, source_bot=None, buyer_session=None):
        ...
        Args:
            ...
            source_bot: 源机器人用户名（如 @hao24bot，可选）
            buyer_session: 代购账号 session 文件路径（可选）
        ...
        # 🎯 如果是 @SanJianbot，使用专属购买器
        if source_bot == '@SanJianbot':
            return await self._purchase_sanjianbot(product_id, quantity, user_id, order_id, buyer_session)
        
        # 否则使用原来的 @hao24bot 流程
        return await self._purchase_hao24bot(product_id, quantity, user_id, order_id)
    
    async def _purchase_hao24bot(self, product_id, quantity, user_id, order_id):
        """@hao24bot 购买流程（原来的逻辑）"""
        # 🔒 全局锁：同一时间只处理一个订单
        async with AutoPurchaser._purchase_lock:
            print(f'🔒 订单 #{order_id} (用户 {user_id}) 开始处理...')
            print(f'📱 使用来源: @hao24bot')
    ...
    async def _purchase_sanjianbot(self, product_id, quantity, user_id, order_id, buyer_session):
        """@SanJianbot 购买流程（使用专属购买器）"""
        # 创建临时客户端
        ...
        async with AutoPurchaser._purchase_lock:
            ...
            try:
                ...
                purchaser = SanJianbotPurchaser(temp_client)
                ...
                if balance < required_amount:
                    raise Exception(f'余额不足！需要: ${required_amount:.2f}, 当前: ${balance:.2f}。请手动充值后重试。')
                
                print(f'✅ 余额充足 (需要: ${required_amount:.2f}, 余额: ${balance:.2f})')
                
                # 记录购买前的最后消息ID
                msgs = await temp_client.get_messages('@SanJianbot', limit=1)
                last_msg_id = msgs[0].id if msgs else 0
                print(f'📍 起始消息ID: {last_msg_id}')
